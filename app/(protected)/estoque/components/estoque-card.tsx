...
// ✅ CORREÇÃO: id agora é number para bater com a API
...
const UpsertEstoqueForm = ({
  ...
  // Removido o estado 'imagem' que não estava a ser usado
  ...
  const uploadImage = async (file: File): Promise<string | null> => {
    try {
      ...
      // Usa a URL da API definida no ambiente ou localhost
      const API_BASE_URL =
        process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080";
  ...
  const handleFileChange = async (
    ...
    if (file) {
      ...
      // Removido setImagem(file) pois não era usado
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const estoqueData: EstoqueData = {
      // ✅ CORREÇÃO: Usa Date.now() (número) se não houver ID, em vez de string
